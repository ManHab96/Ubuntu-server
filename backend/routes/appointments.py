...
async def get_appointments(agency_id: str = None, status: str = None, current_user: dict = Depends(get_current_user)):
    query = {}
    if agency_id:
    ...
    appointments = await appointments_collection.find(query, {"_id": 0}).to_list(1000)
...
async def cancel_appointment(appointment_id: str, current_user: dict = Depends(get_current_user)):
    result = await appointments_collection.update_one({"id": appointment_id}, {"$set": {"status": AppointmentStatus.CANCELLED}})
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="Appointment not found")
